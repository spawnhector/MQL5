   ...
   while(foundBar!=currentBar)
     {
      ...
      //int index_highest2 = ArrayMaximum(prevFoundHigh,(ArraySize(prevFoundHigh)-tpBars/2),MAX_CANDELS);
      //int index_lowest2 = ArrayMinimum(prevFoundLow,(ArraySize(prevFoundLow)-tpBars/2),MAX_CANDELS);
         // ArrayPrint(prevFoundLow);
         ...
         if(barType == "High")
           {
            if((prevHigh != 0.0) && (candels[index_highest].close > Bid) && (Bid == candels[index_lowest].close))
              {
               //Print("serach high",searchArray(ArraySize(openOrder),prevHigh));
               //ArrayPrint(openOrder);
                  ...
                  if(ArraySize(prevFoundLow) > (tpBars))
                    {
                     ...
                     //request.sl=iHigh(Symbol(),Period(), foundBar) + spBars;
                  ...
                  if(ArraySize(prevFoundHigh) > (tpBars))
                    {
                     ...
                     request.symbol=_Symbol;
                     request.volume=lot_size;
                     //request.sl=iLow(Symbol(),Period(),foundBar)-spBars;
